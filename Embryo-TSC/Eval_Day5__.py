...
parser.add_argument("--NN", type=int, default=9)
...
name = './rundataDay5_r/train_day' + (''.join(sys.argv[1:]))

netS = name + '.pth'
out_name = './rundataE/outDay5-{}-{}-{}_'.format(args.Ra, args.lr, args.JobId)
...
Lens_5 = len_cal(e_i1, st_path=day_index_path, image_paths=c_p1)
lenE = np.array([np.quantile(Lens_5, 0.25), np.quantile(Lens_5, 0.5), np.quantile(Lens_5, 0.75), 1000])
lenSt = 1000

epochR = args.epochR


samTr = MySamplerFilterNrepFixDay5(e_i1, lenSt, st_path=day_index_path, image_paths=c_p1)
samVa = MySamplerFilterNrepFixDay5(e_i2, 1000, st_path=day_index_path, image_paths=c_p2)
...
if not args.dd:
    ...
    TrainD = MyDatasetDivideNrep_Day5_order(
        image_paths=c_p1,
        seq_length=seq_length,
...
ValiD = MyDatasetDivideNrep_Day5_order(
    image_paths=c_p2,
    seq_length=seq_length,
    transform=transformV,
    # length=len(e_i2)-1,
...
for epoch in range(1):
    ...
    for phase in ['val', 'train']:
        ...
        if phase == 'val':
            reslog(
                [phase, 1, args.ims, args.nch, args.nlayer, args.Dil, args.tt, args.EnbB, args.epochR, args.SL, args.BS,
        ...
        with h5py.File(os.path.join(save_path, 'Ra_{}_day_{}_'.format(args.Ra, 1) + phase + '_features.h5'),
                       'w') as h5f:
