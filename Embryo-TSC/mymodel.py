from TSCBERT import Classifier_TSCBERT, Inception_TBO
from TimeSeriesClass import *


class MYTSC(nn.Module):
    def __init__(self, mode, input_shape, lenght, hidden_size=128, pow=2, LrEnb=0, LrMo=0, device='cuda', n_cl=1,
        ...
        if mode == 1:
            ...
            self.TSC = Classifier_FCN_FTA(input_shape=input_shape, D=D2, length=lenght, ffh=ffh * 4)
            self.drop = nn.Dropout(p=0.7)
            self.fc = nn.Linear(in_features=self.TSC.out_shape, out_features=n_cl)

        elif mode == 301:

            self.TSC = Classifier_FCN_FTA_B(input_shape=input_shape, D=D2, length=lenght, ffh=ffh * 4)
            self.drop = nn.Dropout(p=0.7)
            self.fc = nn.Linear(in_features=self.TSC.out_shape, out_features=n_cl)
        elif mode == 400:
            ...
            self.TSC = Classifier_RESNET_FTA1_B(input_shape=input_shape, D=D2, length=lenght, ffh=ffh * 4)
            self.drop = nn.Dropout(p=0.7)
            self.fc = nn.Linear(in_features=self.TSC.out_shape, out_features=n_cl)
            ...
            self.TSC = Inception_TBO(type=mode % 10,
                                     ...
                                     ffh=ffh,
                                     in_channels=input_shape, n_filters=hidden_size // 4, kernel_sizes=[9, 19, 39],
